...
`useMutation` 훅은 정의한 뮤테이션에 대한 다양한 값을 반환한다. 반환되는 값들은 뮤테이션을 실행하거나, 상태 및 결과를 관리하고 이를 기반으로 UI를 제어할 수 있다. 대표적으로 다음과 같다.
...
각 쿼리의 캐시는 `queryKey`를 기준으로 식별되며 관리된다. `useQuery`에서는 "캐시는 쿼리 단위로 관리한다, `queryKey`를 통해 구분한다"고 했는데, 결국 `queryKey`는 캐시를 관리하기 위한 기준이라는 의미이다. 따라서 동일한 `queryKey`를 사용하는 쿼리가 여러 개 존재하더라도, 캐시는 하나만 생성되고 재사용된다.
...
이를 위해 react-query는 4가지 주요 캐싱 상태를 정의한다.
...
- 윈도우 포커스가 돌아오거나 네트워크가 재연결됬을 때, 현재 화면에서 사용중인 쿼리의 캐시가 `stale` 상태인 경우
...
데이터를 가져온 쿼리가 더 이상 어떤 컴포넌트에서도 사용되지 않는다면, 해당 쿼리는 `inactive` 상태가 된다. 모든 컴포넌트에서 구독이 해제되면(해당 쿼리를 사용하는 컴포넌트가 모두 언마운트 되면) 쿼리는 더 이상 사용되지 않는 상태가 되며, 이 시점부터 `inactive` 상태로 관리된다.
...
`inactive` 상태가 되었다고 해서 즉시 캐시가 제거되는 것은 아니다. 일정 시간 동안 `inactive` 상태로 유지되다가, 시간이 지나면 캐시에서 제거된다. 이를 `gcTime`이라고 부르며, 기본값은 5분(1000 _ 60 _ 5)이다. `gcTime` 역시 `QueryClient`를 통해 전역 설정을 하거나 각 쿼리마다 별도로 설정할 수 있다.
...
하지만, 캐시된 데이터는 항상 서버의 데이터와 일치하는 것은 아니다. 요청에 의해 서버의 데이터가 변경되었을 때, 캐시된 데이터는 여전히 이전 상태일 수 있다. 따라서 특정 시점에 캐시를 무효화하고 다시 데이터를 가져오는 전략이 필요하다.
...
`invalidateQueries`의 `queryKey`는 특정 쿼리를 정확히 지정하는 것이 아니라, 해당 키를 포함하는 모든 쿼리를 대상으로 한다. 즉, `queryKey`가 부분적으로 일치하는 모든 쿼리가 무효화된다. 따라서, 위의 코드는 `['item']`, `['item', 1]`, `['item', 'active']` 등 `['item']`을 포함하는 모든 쿼리를 무효화시킨다. 따라서, 네임스페이스처럼 쿼리를 그룹화하고 한 번에 무효화할 수도 있다.
...
TODO 목록을 불러오는 `/` 페이지에서, 위와 같은 쿼리를 사용하고 있다고 가정해보자. `staleTime`이 5분으로 설정되어 있으므로, 쿼리가 처음 실행되고 나서 5분 동안은 캐시된 데이터를 재사용한다.

이 때, 만약 `/add` 페이지에서 새로운 TODO 아이템을 추가하는 경우 어떻게 될까?
...
![사진 11) TODO 목록이 갱신되지 않는다](../images/study_about_react_query/not_refresh_todo_list.gif)

`['todos']` 쿼리의 `staleTime`은 5분으로 설정되어 있다. 이 시간이 지나지 않았을 경우 캐시가 `fresh` 상태로 유지되고 있기 때문에, 쿼리가 다시 실행되더라도 네트워크 요청이 발생하지 않고 캐시된 데이터를 재사용한다. 따라서 새로운 아이템이 추가되었음에도 불구하고, TODO 목록은 갱신되지 않는다.
