...
단순히 API 요청을 처리해 데이터를 가져오는 역할만 하는 것이 아니라, 로딩, 에러, 데이터 상태를 자동으로 관리해준다. 그래서 앞서 봤던 `useEffect`와 `useState`를 직접 조합하는 것보다 훨씬 간결하게 사용할 수 있다. 또한, 중복 요청을 방지하며 캐싱 기능을 제공해 불필요한 네트워크 요청을 줄이는 데에도 도움을 준다.
...
`queryKey` 배열은 복수개의 아이템이 포함될 수 있으며, 이를 통해 쿼리를 더욱 세분화 할 수 있다. `["todos", "create"]`와 같이 배열에 여러 개의 요소를 포함하여, 쿼리를 구분할 수 있다.
...
배열의 각 요소에는 문자열 뿐만 아니라 숫자, 불리언, 객체 등 다양한 타입을 사용할 수 있다. 그래서 동적인 값을 포함하여 쿼리를 구분하여, 특정 변수에 따라 세분화할 수 있다. 이 때, `queryKey`에 포함된 값이 변경되면, react-query는 이를 새로운 쿼리로 인식하고, `queryFn`을 다시 실행하여 데이터를 가져온다.
...
![사진 1) 한 개만 보인다.](../images/study_about_react_query/display_only_one_query.png)

> 정확히 어떤 비교를 수행하는지 찾아보려 했으나, 추측하기로는 [@tanstack/react-query - hashKey](https://github.com/TanStack/query/blob/main/packages/query-core/src/utils.ts#L216) 함수 통해 문자열로 변환 후 비교하는 것으로 보인다.
...
쿼리 취소는 `useQuery`가 언마운트되거나 `queryKey`가 변경될 때, 혹은 `QueryClient`를 통해 쿼리를 직접 취소할 때 발생한다. 직접 취소는 기회가 될 때 다뤄보고자 한다.

`select`는 쿼리의 결과 데이터를 가공하는 함수이다. `queryFn`에서 반환된 데이터를 받아서, 컴포넌트에서 필요한 형태로 가공하여 반환할 수 있다.

```ts
const { data: todoTitles = [], isFetching } = useQuery({
  queryKey: ["todos"],
  queryFn: () => getTodos(),
...
`queryFn`에서도 데이터를 가공할 수 있겠지만, 쿼리 캐시는 `queryFn` 에서 반환된 원본 데이터를 기준으로 관리된다. 그래서 데이터 가공은 `select` 사용이 권장되며, 이를 통해 캐시된 원본 데이터를 유지하면서 컴포넌트에서 필요한 형태로 데이터를 가공하여 사용할 수 있다.
