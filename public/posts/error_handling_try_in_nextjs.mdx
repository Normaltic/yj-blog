---
title: Next.js 서버 컴포넌트의 요청 실패 핸들링 해보기
...
![사진 1) 기본 화면](../images/error_handling_try_in_nextjs/default_page_screen.png)
...
![사진 2) 기본 에러](../images/error_handling_try_in_nextjs/default_error_screen.png)
...
![사진 3) error page 적용](../images/error_handling_try_in_nextjs/default_error_page.png)
...
![사진 4) ErrorBoundary 적용](../images/error_handling_try_in_nextjs/default_error_boundary_screen.png)
...
먼저 `ServerRandomNumber` 컴포넌트에서 데이터를 요청한다. 요청이 정상적으로 실행됬을 경우 결과물을 출력한다. 그리고 요청에 실패하면, 이전에 작성했던 `ClientRandomNumber` 반환하여 클라이언트에서 처리하도록 한다.
...
![사진 5) 함수는 직접 전달할 수 없다](../images/error_handling_try_in_nextjs/function_cannot_passed_directly.png)
...
물론 모든 함수를 무조건 넘겨주지 못하는건 아닌데, 만약 함수가 Next.js의 서버 액션이라면 `use server` 디랙티브를 통해 해당 함수가 서버에서 실행될 함수임을 명시해주면 가능하다. 하지만 보통 form action이나 서버 함수를 클라이언트에서 호출하기 위한 용도로 사용되기 때문에, 여기서 사용하려는 구조처럼 클라이언트 컴포넌트로 함수를 `props`로 넘겨 실행하려는 상황에서는 적합하지 않다.
...
`router.refresh` 메서드는 이름 그대로 새로 고침을 하는 메서드로, Next.js의 `useRouter`를 통해 `router` 객체에 접근할 수 있다. 비슷하게 새로 고침은 `window.location.reload`로도 수행할 수 있는데, 두 메서드에는 중요한 차이점이 존재한고 한다.
...
![사진 6) 새로 고침](../images/error_handling_try_in_nextjs/errorboundary_with_refresh.gif)
...
![사진 7) 하나 더](../images/error_handling_try_in_nextjs/one_more_random_number.png)

이 때, 에러가 난 부분에 대해서 다시 시도하기 위해 Retry 버튼을 눌러 새로 고침을 수행했다. 무슨 일이 일어날까?

![사진 8) 다 같이 업데이트](../images/error_handling_try_in_nextjs/one_more_random_number_refresh.gif)
